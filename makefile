...
build/%.o: build/%.c
	clang $(WARNS) -c -O0 -g -o $@ $< -Iinclude -Wno-discarded-qualifiers
...
src/boards_compressed.c: misc/compress_boards.c
	mkdir -p build
	clang $(WARNS) -O0 -g -Iinclude misc/compress_boards.c src/boards.c -o build/generate_compressed_builds
